"""Upscale stage card for PipelinePanelV2."""
...
class UpscaleStageCard(ttk.Frame):
    """Stage card for upscale settings."""
    ...
    def __init__(self, master: tk.Misc, *, controller=None, theme=None, **kwargs) -> None:
        ...
        self.header_label = ttk.Label(self, text="Upscale Settings", style=header_style)
        ...
        for idx, field in enumerate(self.FIELD_NAMES):
            ...
            elif field == "denoising_strength":
                self._add_spinbox(self.body, field, var, idx, from_=0.0, to=1.0, increment=0.05)
            else:
    ...
    def to_config_dict(self) -> dict:
        ...
        return {"upscale": section} if section else {}

    @staticmethod
    def _get_section(config: dict | None) -> dict:
        if isinstance(config, dict):
            section = config.get("upscale") or {}
