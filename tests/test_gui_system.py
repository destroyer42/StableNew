...
class GUITestValidator:
    ...
    def test_pack_config_persistence(self):
        ...
        matches = (loaded_config.get("txt2img", {}).get("steps") == 30 and
                  loaded_config.get("txt2img", {}).get("cfg_scale") == 7.5)
        
        self.log_test(
            "Pack Config Persistence",
            saved and matches,
            f"Save: {saved}, Load matches: {matches}"
        )
    ...
    def test_config_validation(self):
        ...
        has_txt2img_fields = all(field in test_config.get("txt2img", {}) for field in required_txt2img)
