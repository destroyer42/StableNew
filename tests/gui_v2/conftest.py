...
def gui_app_factory(monkeypatch: pytest.MonkeyPatch, tk_root: tk.Tk, tmp_path):
    ...
    for directory in (packs_dir, presets_dir, lists_dir, output_dir):
        directory.mkdir(parents=True, exist_ok=True)

    messagebox_stub = _create_messagebox_stub()
    monkeypatch.setattr("src.gui.main_window.messagebox", messagebox_stub)
    monkeypatch.setattr("src.gui.main_window.launch_webui_safely", lambda *_, **__: False)
    monkeypatch.setattr("src.gui.main_window.find_webui_api_port", lambda *_args, **_kwargs: None)
    simpledialog_stub = SimpleNamespace(askstring=lambda *_, **__: None)
    monkeypatch.setattr("src.gui.main_window.simpledialog", simpledialog_stub)
