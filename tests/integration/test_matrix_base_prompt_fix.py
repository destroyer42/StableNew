...
def test_heroes_portrait_matrix_config():
    ...
    assert not any("knight" in v.text for v in variants), "Pack prompt should be replaced by base_prompt"
    assert not any("armor" in v.text for v in variants), "Pack prompt should be replaced by base_prompt"
    
    # Verify labels track matrix substitutions
    labels = [v.label for v in variants]
    assert any("job" in (label or "") for label in labels), "Labels should track matrix slot replacements"
